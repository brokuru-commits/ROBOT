...
def draw_status_icons(t):
    ...
    for i in range(3): pygame.draw.arc(s_w, (0, 180, 255, w_v), (5+i*4, 8+i*4, 24-i*8, 24-i*8), math.pi/4, 3*math.pi/4, 2)
    screen.blit(s_w, (start_x, y))
    # Rad - extrem langsam
    r_v = int(100 + 100 * math.sin(t * 0.5))
    s_r = pygame.Surface((34,34), pygame.SRCALPHA); c=(17,17)
    pygame.draw.circle(s_r, (255, 170, 0, r_v), c, 4)
    for i in range(3):
        ...
        pygame.draw.polygon(s_r, (255, 170, 0, r_v), [c, p1, p2])
    screen.blit(s_r, (start_x + 35, y))
    # Blitz - Seltenes Zucken
    bc = (255, 255, 100, 255) if random.random() > 0.98 else (140, 140, 0, 100)
...
next_event = time.time() + 180 # Erstes Event nach 3 Min
...
while True:
    ...
    if not active_event and t_now > next_event:
        ...
        next_event = t_now + random.randint(180, 600) # 3-10 Min Pause
